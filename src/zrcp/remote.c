...
{"tbblue-set-register",NULL,"index value","Set TBBlue register with value at index"},
 ...
 {"tsconf-set-af-port",NULL,"index value","Set TSConf XXAF port value"},
  ...
  {"zeng-online","|zo","command parameter","Related ZENG Online actions. Command can be:\n"
    ...
    "streaming-get-audio-cont user_pass n            Same as streaming-get-audio but this command returns continuously (never ends). Every one separated by NL character.\n"
    "                                                If there is not any stream to get, it will block until one is generated\n"
...
//Comprobar longitud condicion. Si condicion vacia, acabara desactivando el breakpoint
  ...
  int result=debug_set_breakpoint(indice-1,&parametros[i],0);
            ...
            do {
                ...
                */
                ...
                if (leidos>0) {
                    ...
                    if (remote_protocol_char_mode.v) {
                        if (leidos==1 && buffer_lectura_socket[indice_destino]==0x7f) {
